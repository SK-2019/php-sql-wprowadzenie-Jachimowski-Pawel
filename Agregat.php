...
worker(12,"SELECT SUM(zarobki) as suma_zarobki, if( (imie not like '%a'), 'men','kobiety') as 'plec' FROM `pracownicy` GROUP by (imie like '%a')","Suma zarobków kobiet i mężczyzn* ","suma_zarobki","plec");
worker(13,"SELECT AVG(zarobki) as srednia_zarobki, if( (imie not like '%a'), 'men','kobiety') as 'plec' FROM `pracownicy` GROUP by (imie like '%a')","Średnia zarobków kobiet i mężczyzn* ","srednia_zarobki","plec");
?>
?>
